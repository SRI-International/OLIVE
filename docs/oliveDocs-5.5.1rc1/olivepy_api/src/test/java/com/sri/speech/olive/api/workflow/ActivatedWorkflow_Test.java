...
public class ActivatedWorkflow_Test {
    ...
    public void testRunBasicWorkflow() throws Exception {
        ...
        Server.ResultCallback<Workflow.WorkflowAnalysisRequest, Workflow.WorkflowAnalysisResult> rc = new Server.ResultCallback<Workflow.WorkflowAnalysisRequest, Workflow.WorkflowAnalysisResult>() {
            ...
            @Override
            public void call(Server.Result<Workflow.WorkflowAnalysisRequest, Workflow.WorkflowAnalysisResult> r) {
    ...
    //                        System.out.println("");
        ...
        Olive.Audio.Builder audio = ClientUtils.createAudioFromFile(AUDIO_TEST_FILE,
                ...
                new ArrayList<>());
        Workflow.WorkflowDataRequest wdr = ow.packageAudio(audio.build(), AUDIO_TEST_FILE);

        // OR
        Olive.BinaryMedia.Builder media = ClientUtils.createBinaryMediaFromFile(AUDIO_TEST_FILE,
                ClientUtils.AudioTransferType.SEND_SERIALIZED_BUFFER,
                new ArrayList<>());
        Workflow.WorkflowDataRequest mdr = ow.packageBinaryMedia(media.build(), AUDIO_TEST_FILE);

        List<Workflow.WorkflowDataRequest> dataList = new ArrayList<>();
        dataList.add(wdr);
    ...
    public void testWorkflowClasses() throws Exception {
        ...
        // Prepare a callback:
        Server.ResultCallback<Workflow.WorkflowClassStatusRequest, Workflow.WorkflowClassStatusResult> rc = new Server.ResultCallback<Workflow.WorkflowClassStatusRequest, Workflow.WorkflowClassStatusResult>() {

            @Override
            public void call(Server.Result<Workflow.WorkflowClassStatusRequest, Workflow.WorkflowClassStatusResult> r) {

//                Map<String, JobResult> jobResults = WorkflowUtils.extractWorkflowAnalysis(r.getRep());

                // do something with the results:
                if (!r.hasError()) {
                    for (Workflow.JobClass jobs : r.getRep().getJobClassList()) {
                        // print results
                        System.out.println(String.format("Job %s has %d tasks:",jobs.getJobName(), jobs.getTaskCount()));
                        for (Workflow.TaskClass tc : jobs.getTaskList()){
        ...
        // OR
//        ow.currentClasses(rc, Workflow.WorkflowType.WORKFLOW_ENROLLMENT_TYPE);
