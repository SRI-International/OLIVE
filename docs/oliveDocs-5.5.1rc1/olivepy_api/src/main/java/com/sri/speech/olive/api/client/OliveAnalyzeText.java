...
import com.sri.speech.olive.api.utils.parser.TextParser;
import com.sri.speech.olive.api.utils.reports.AbstractReportWriter;
import com.sri.speech.olive.api.utils.reports.ReportException;
import com.sri.speech.olive.api.utils.reports.TaskReportManager;
import com.sri.speech.olive.api.utils.reports.TextTransformerReportWriter;
...
//    private static RegionParser regionParser = new RegionParser();
    private static ArrayList<String> textInput = new ArrayList<String>();
    private static String textInputFilename;
    // options
    ...
    private static CommandLine parseCommandLineArgs(String args[]) {
        ...
        // text input options (mututally exclusive)
        options.addOption(inputOptionStr,     "input",      true, "NAME of the input file");
...
//        options.addOption(Option.builder().longOpt(thresholdOptionStr).desc("Apply threshold NUMBER when scoring").hasArg().build());
        ...
        if (cmd.hasOption(textOptionStr) ) {
            textInput.add(cmd.getOptionValue(textOptionStr));
        }
        else {
        ...
        if (cmd.hasOption(inputOptionStr) ) {
            if (textInput.size() != 0) {
                System.err.println("Arguments '"+ textOptionStr + "' '" + inputOptionStr + "' are mutually exclusive.  Use only one and run again.");
                printUsageAndExit(options);
            }
            String textFileName = cmd.getOptionValue(inputOptionStr);
            // Make sure file exists
            if (!Files.exists(Paths.get(textFileName).toAbsolutePath())){
                System.err.println("ERROR: input file '" + textFileName + "' does not exist");
                printUsageAndExit(options);
            }
            textInputFilename = cmd.getOptionValue(inputOptionStr);
            try {
                for (String line : Files.readAllLines(Paths.get(textInputFilename))) {
                    textInput.add(line);
                }
            } catch (IOException e) {
                System.err.println("Invalid input file: " + textInputFilename);
                printUsageAndExit(options);
            }
        }
        System.out.println("Final text input = ");
    ...
    public void handleRequests(Server server){
        ...
        try {
            ...
            for (String line : textInput) {
                if (requestTextTransformation(server, pd, line, true)){
                    numAsyncRequest++;
                }
            }
...
//
    ...
    private boolean requestTextTransformation(Server server, Pair<Olive.Plugin, Olive.Domain> plugin, String text, boolean runAsync) throws ClientException, IOException, UnsupportedAudioFileException {
        ...
        Server.ResultCallback<Olive.TextTransformationRequest, Olive.TextTransformationResult> rc = new Server.ResultCallback<Olive.TextTransformationRequest, Olive.TextTransformationResult>() {
            ...
            public void call(Server.Result<Olive.TextTransformationRequest, Olive.TextTransformationResult> r) {
                ...
                if(!r.hasError()){
                    ...
                    // SAVE OUTPUT
                    try {
                        Pair<TaskType, String> key = new Pair<>(TaskType.TXT_TRANSFORM, "");
                        AbstractReportWriter writer = rptMgr.getWriter(key, () -> new TextTransformerReportWriter(outputDirName, ""));
                        ((TextTransformerReportWriter)(writer)).addData(r.getRep().getTransformationList());
        ...
        System.out.println(String.format("Submitting %s for translation with plugin %s and domain %s", text, plugin.getFirst().getId(), plugin.getSecond().getId()));

        return ClientUtils.requestTextTransformation(server, plugin, text, rc, runAsync, propertyOptions, classIDs);
