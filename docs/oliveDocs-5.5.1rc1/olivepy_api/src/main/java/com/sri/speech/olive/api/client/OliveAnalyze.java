...
//        options.addOption(Option.builder().longOpt(printClassesOptionStr).desc("Print class names if also printing plugin/domain names.  Must use with --print option.  Default is to not print class IDs").build());
        ...
        // Update options
        options.addOption(Option.builder().longOpt(applyUpdateOptionStr).desc("Request the plugin is updated (if supported)").build());
...
//                        }
            ...
            while(numAsyncRequest > 0){
                ...
                // TODO I THINK WE NEED TO REMVOE THIS?  The server should tell us the request is still active
                if(null == sr) {
                    if (numTimeouts++ > 3) {
//                        System.err.println("Timeout exceeded waiting for response... ");
                        System.err.println("Still waiting for a response... Request is taking longer than expected, check server status or ignore if large job");
                        numTimeouts = 0;
                        //numErrors++;
                        //break;
                    }
...
*/
    ...
    /*public boolean requestWorkflow(Server server*//*, Pair<Olive.Plugin, Olive.Domain> pp*//*) throws ClientException, IOException, UnsupportedAudioFileException {
        ...
        // Create a callback to handle async SAD results from the server, note that we pass sadRequstId to the callback as an example of bundling metadata with the score results
        Server.ResultCallback<Workflow.WorkflowAnalysisRequest, Workflow.WorkflowAnalysisResult> rc = new Server.ResultCallback<Workflow.WorkflowAnalysisRequest, Workflow.WorkflowAnalysisResult>() {

            @Override
            public void call(Server.Result<Workflow.WorkflowAnalysisRequest, Workflow.WorkflowAnalysisResult> r) throws InvalidProtocolBufferException {
                // do something with the results:
                if(!r.hasError()){
                    ...
                    if(r.getRep().hasError()) {
                        ...
                        for(Workflow.WorkflowTaskResult wkflwResult : r.getRep().getJobResult(0).getTaskResultsList()){
...
//         workflowDef.setProcessingType(Workflow.WorkflowJobType.MERGE);
        ...
        Workflow.WorkflowJob.Builder workflowJobDef = Workflow.WorkflowJob.newBuilder();
        workflowJobDef.setWorkflowType(Workflow.WorkflowType.ANALYSIS);
...
//        workflowDef.addJobs(workflowJobDef);
        ...
        // Hack
//        Workflow.Workflow.Builder workflowBuildr = Workflow.Workflow.newBuilder().addJobs(workflowJobDef);
...
//                .setDataLabel("audio");  // todo test with another label..
        ...
        Workflow.WorkflowAnalysisRequest.Builder req = Workflow.WorkflowAnalysisRequest.newBuilder();
