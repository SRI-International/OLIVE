...
class Package(Command):
    ...
    def execute(self, command, capture_output=False):
        ...
        print("running cmd: {}".format(shlex.split(command)))
        process = subprocess.Popen(shlex.split(command), stdout=subprocess.PIPE)

        print("reading stdout...")
        while True:
            output = process.stdout.readline()
            print("got stdout: {}".format(output))

            rtn_code = process.poll()
            print("rtn code: {}".format(rtn_code))
            # if output == "" and  rtn_code is not None:
            if output == b"" and  rtn_code == 0:
                print("break")
                break
        ...
        if return_code != 0:
            print("Error running command {} - exit code: {}".format(command, return_code))
    ...
    def run(self):
        commands = []
        commands.extend([
            "rm -rf {}".format(WHEELHOUSE),
            "mkdir -p {}".format(WHEELHOUSE),
            "pip3 wheel --wheel-dir={} -r requirements.txt".format(WHEELHOUSE)
        ]
        )
