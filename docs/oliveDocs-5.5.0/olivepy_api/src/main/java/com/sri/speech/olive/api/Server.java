...
final public class Server {
    ...
    public final class ServerInfo implements Cloneable {
        ...
        public Set<PluginError> failedPlugins = new HashSet<>();
        ...
        protected void update(Heartbeat heartbeat) {
            if (heartbeat.hasStats()) {
                ...
                this.loadedPlugins.addAll(stats.getLoadedPluginsList());
                this.failedPlugins.addAll(stats.getErrorPluginsList());
        ...
        public ServerInfo clone() {
            ...
            that.loadedPlugins.addAll(this.loadedPlugins);
            that.failedPlugins.addAll(this.failedPlugins);
            return that;
        }
        ...
        public String toString() {
            ...
            StringBuilder loaded_sb = new StringBuilder();
            for (Plugin p : loadedPlugins){
                for (Domain d : p.getDomainList()) {
                    loaded_sb.append(p.getId()).append(":").append(d.getId()).append(",");
                }
            }
            if(loaded_sb.length()>0){
                loaded_sb.deleteCharAt(loaded_sb.length()-1);
            }

            StringBuilder failed_sb = new StringBuilder();
            for (PluginError p : failedPlugins){
                failed_sb.append(p.getPluginId()).append(":").append(p.getDomainId()).append(" - ").append(p.getError()).append(",");
            }
            if(failed_sb.length()>0){
                failed_sb.deleteCharAt(failed_sb.length()-1);
            }
            return String.format("[ServerInfo: version: %s cpu-percent=%1.2f cpu-average=%1.2f mem-percent=%1.2f " +
                    ...
                    "pool-pending=%d pool-finished=%d pool-reinit=%b log-dir=%s, loaded-plugins=[%s], failed-plugins=[%s]]",
                    version, cpuPercent, cpuAverage, memPercent, maxMemPercent, swapPercent, maxSwapPercent, numberWorkers, poolBusy,
                    poolPending, poolFinished, poolReinit, logdir, loaded_sb.toString(), failed_sb.toString());
        }
    ...
    private void processIncomingMessage(ScenicMessage rep) throws InvalidProtocolBufferException {
        if (rep.getMessageType() == MessageType.GET_ACTIVE_RESULT) {
            ...
            if (rep.getMessageId().equals(pending_status_request_id)) {
                ...
                //logger.debug("active messages: {}", active.getMessageIdList());
            ...
            try {
                ...
                if (items.pollin(0)) {
                    ...
                    if (!connected.get()) {
                        ...
                        logger.debug((disconnected ? "Reconnected" : "Connected") + " to " + servStr);
                        synchronized(listeners) {
                            for (ConnectedListener l : listeners)
...
//    }
    ...
    public static final Deserializer[]  deserializers = {
        ...
        @Override
           ...
           new Deserializer() {//77 *
                ...
                @Override
                public Message run(ByteString bytestr) throws InvalidProtocolBufferException {
                    return StartStreamingRequest.parseFrom(bytestr);
                }
            },new Deserializer() {//85 *
                @Override
                public Message run(ByteString bytestr) throws InvalidProtocolBufferException {
                    return StartStreamingResult.parseFrom(bytestr);
                }
            },new Deserializer() {//86 *
                @Override
                public Message run(ByteString bytestr) throws InvalidProtocolBufferException {
                    return Audio.parseFrom(bytestr);
                }
            },new Deserializer() {//87 *
                @Override
                public Message run(ByteString bytestr) throws InvalidProtocolBufferException {
                    return RegionScorerStreamingRequest.parseFrom(bytestr);
                }
            },new Deserializer() {//88 *
                @Override
                public Message run(ByteString bytestr) throws InvalidProtocolBufferException {
                    return RegionScorerStreamingResult.parseFrom(bytestr);
                }
            },new Deserializer() {//89 *
                @Override
                public Message run(ByteString bytestr) throws InvalidProtocolBufferException {
                    return GlobalScorerStreamingRequest.parseFrom(bytestr);
                }
            },new Deserializer() {//90 *
                @Override
                public Message run(ByteString bytestr) throws InvalidProtocolBufferException {
                    return GlobalScorerStreamingResult.parseFrom(bytestr);
                }
            },new Deserializer() {//91 *
                @Override
                public Message run(ByteString bytestr) throws InvalidProtocolBufferException {
                    return TextTransformationStreamingRequest.parseFrom(bytestr);
                }
            }
            ,new Deserializer() {//92 *
                @Override
                public Message run(ByteString bytestr) throws InvalidProtocolBufferException {
                    return TextTransformationStreamingResult.parseFrom(bytestr);
                }
            },new Deserializer() {//93 *
                @Override
                public Message run(ByteString bytestr) throws InvalidProtocolBufferException {
                    return StopStreamingRequest.parseFrom(bytestr);
                }
            },new Deserializer() {//94 *
                @Override
                public Message run(ByteString bytestr) throws InvalidProtocolBufferException {
                    return StopStreamingResult.parseFrom(bytestr);
                }
            },new Deserializer() {//95 *
                @Override
                public Message run(ByteString bytestr) throws InvalidProtocolBufferException {
                    return FlushStreamingRequest.parseFrom(bytestr);
                }
            },new Deserializer() {//96 *
                @Override
                public Message run(ByteString bytestr) throws InvalidProtocolBufferException {
                    return FlushStreamingResult.parseFrom(bytestr);
                }
            },new Deserializer() {//97 *
                @Override
                public Message run(ByteString bytestr) throws InvalidProtocolBufferException {
                    return SyncStreamingRequest.parseFrom(bytestr);
                }
            },new Deserializer() {//98 *
                @Override
                public Message run(ByteString bytestr) throws InvalidProtocolBufferException {
                    return DrainStreamingRequest.parseFrom(bytestr);
                }
            },new Deserializer() {//99 *
                @Override
                public Message run(ByteString bytestr) throws InvalidProtocolBufferException {
                    return DrainStreamingResult.parseFrom(bytestr);
                }
            },

            null, // INVALID_MESSAGE (error only) // 100
    };
    ...
    static
    {
        ...
        type_map.put(DataOutputTransformRequest.class, MessageType.DATA_OUTPUT_TRANSFORMER_REQUEST);
        type_map.put(DataOutputTransformResult.class, MessageType.DATA_OUTPUT_TRANSFORMER_RESULT);
        ...
        type_map.put(WorkflowBinaryMediaResult.class, MessageType.BINARY_MEDIA_RESULT);
        type_map.put(StartStreamingRequest.class, MessageType.START_STREAMING_REQUEST);
        type_map.put(StartStreamingResult.class, MessageType.START_STREAMING_RESULT);
        type_map.put(Audio.class, MessageType.AUDIO_STREAM);
        type_map.put(RegionScorerStreamingRequest.class, MessageType.REGION_SCORER_STREAMING_REQUEST);
        type_map.put(RegionScorerStreamingResult.class, MessageType.REGION_SCORER_STREAMING_RESULT);
        type_map.put(GlobalScorerStreamingRequest.class, MessageType.GLOBAL_SCORER_STREAMING_REQUEST);
        type_map.put(GlobalScorerStreamingResult.class, MessageType.GLOBAL_SCORER_STREAMING_RESULT);
        type_map.put(TextTransformationStreamingRequest.class, MessageType.TEXT_TRANSFORMER_STREAMING_REQUEST);
        type_map.put(TextTransformationStreamingResult.class, MessageType.TEXT_TRANSFORMER_STREAMING_RESULT);
        type_map.put(StopStreamingRequest.class, MessageType.STOP_STREAMING_REQUEST);
        type_map.put(StopStreamingResult.class, MessageType.STOP_STREAMING_RESULT);
        type_map.put(FlushStreamingRequest.class, MessageType.FLUSH_STREAMING_REQUEST);
        type_map.put(FlushStreamingResult.class, MessageType.FLUSH_STREAMING_RESULT);
        type_map.put(SyncStreamingRequest.class, MessageType.SYNC_STREAMING_REQUEST);
        type_map.put(DrainStreamingRequest.class, MessageType.DRAIN_STREAMING_REQUEST);
        type_map.put(DrainStreamingResult.class, MessageType.DRAIN_STREAMING_RESULT);
