...
//        req.addModifications(createAudioFromFile("/Users/e24652/audio/sad_smoke.wav", channelNumber, transferType, regions));
  ...
  public static Olive.Audio.Builder createAudioFromFile(
    ...
    switch (transferType) {
      ...
      case SEND_SAMPLES_BUFFER:
        ...
        return createAudioFromDecodedBytes(filename, samples,
                ais.getFormat().getChannels(),
                (int)ais.getFormat().getSampleRate(),
                channelNumber,
                ClientUtils.getAudioDepth(ais.getFormat()),
                regions);
  ...
  public static Olive.Audio.Builder createAudioFromDecodedBytes(
            String label,
            byte[] samples ,
            int numberChannels,
            int sampleRate,
            int channelNumber,
            Olive.AudioBitDepth bitDepth,
            List<RegionWord> regions)
            throws IOException, UnsupportedAudioFileException {
    ...
    /**
     * Send a stop streaming request
     * @param server the OLIVE server
     * @param sessionID if specfied stop the streaming session with this ID, otherwise stop all active streams
     * @return
     * @throws ClientException
     * @throws IOException
     * @throws UnsupportedAudioFileException
     */
    public static boolean requestStopStreaming(Server server,
                                             String sessionID) throws ClientException, IOException, UnsupportedAudioFileException {
