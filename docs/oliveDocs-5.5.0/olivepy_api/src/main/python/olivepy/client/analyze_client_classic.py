...
if __name__ == '__main__':
    ...
    if not args.enhance and not args.frame and not getattr(args, 'global') and not args.region and not args.box and not args.text:
    ...
    try:
        for input in data_input:
            try:
                ...
                if args.text:
                    for txt in input.text:
                        texts_out = client.analyze_text_transformation(args.plugin, args.domain, txt, opts=plugin_options, classes=class_ids)
                        if len(texts_out) == 0:
                            print('No translation returned')
                        for txt_result in texts_out:
