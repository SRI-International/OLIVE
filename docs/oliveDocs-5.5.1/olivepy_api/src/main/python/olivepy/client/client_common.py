...
def convert_filename_to_data(input_in, transfer_mode, expected_data_type=OliveInputDataType.AUDIO_DATA_TYPE, check_path=True, selected_channel=None, annotations=None,):
    ...
    if expected_data_type == OliveInputDataType.AUDIO_DATA_TYPE:
        ...
        msgutil.package_audio(audio_msg=audio, audio_data=filename, mode=transfer_mode, label=os.path.basename(filename), selected_channel=selected_channel, annotations=annotations)
        return audio
        ...
        filename = os.path.expanduser(input_in)
        msgutil.package_binary_media(media_msg, filename, mode=transfer_mode, validate_local_path=check_path, label=os.path.basename(filename), selected_channel=selected_channel, annotations=annotations)
        return  media_msg
    ...
    if args.image:
        if not data_input:
            ...
            if not os.path.exists(os.path.expanduser(args.image)):
                print('Your image input file ({}) was not found.'.format(args.image))
                exit(1)
...
def parse_data_list2(list_filename, transfer_mode, expected_data_type=OliveInputDataType.AUDIO_DATA_TYPE, has_class_ids=False):
    try:
        ...
        # files.extend([line.strip() for line in open(list_filename).readlines()])
        # Make sure we don't add empty lines as input
        for line in open(list_filename).readlines():
            line = line.strip()
            if line:
                files.append(line)
            else:
        ...
        if expected_data_type == OliveInputDataType.TEXT_DATA_TYPE:
            # text input - return strings read from file as Text messages
            return [convert_filename_to_data(a.strip(), transfer_mode, expected_data_type) for a in files]
