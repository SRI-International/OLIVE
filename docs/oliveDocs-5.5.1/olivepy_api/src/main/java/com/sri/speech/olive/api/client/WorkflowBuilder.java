...
public class WorkflowBuilder {
  ...
  static final String pemOptionStr = "pem";
  ...
  static final String batchOptionStr =
      "nobatch"; // if using pem or list input files then if set process serially, not in batchs by
                 // class ID
  ...
  // importing)
  private static Logger log = LoggerFactory.getLogger(WorkflowBuilder.class);
  ...
  // By defualt we assume we have binary data:
  private static ClientUtils.DataType dataType = ClientUtils.DataType.BINARY_DATA;
  ...
  public static void main(String[] args) throws Exception {
    ...
    // Perform task(s)
    WorkflowBuilder sc = new WorkflowBuilder();
  ...
  private static CommandLine parseCommandLineArgs(String args[]) {
    ...
    options.addOption(
        Option.builder()
            .longOpt(thresholdOptionStr)
            .desc("Apply threshold NUMBER when scoring")
            .hasArg()
            .build());
    ...
    options.addOption(
        ...
        "wav",
        true,
        "NAME of the wav file (some pluigns may let you specify a audio vector instead)");
    options.addOption(
        pemOptionStr,
        "pem",
        true,
        "NAME of the PEM file to use to read audio inputs for batch enrollment with annotated regions");
    options.addOption(
        Option.builder()
            .longOpt(listOptionStr)
            .desc(
                "Batch enroll using this input list FILE having multiple filenames/class IDs. If you need regions with you class enrollemnts then use the PEM option")
            .hasArg()
            .build());
    options.addOption(
        Option.builder()
            .longOpt(batchOptionStr)
            .desc(
                "Disable batch enrollment when using pem or list input files, so that files are processed serially")
            .build());
    options.addOption(
        audioVectorOptionStr,
        "vec",
        true,
    ...
    options.addOption(
        Option.builder()
            .longOpt(printOptionStr)
            .desc(
                "Print all plugins and domains that suport enrollment and/or class import and export")
            .build());
    options.addOption(
        Option.builder()
    ...
    options.addOption(
        Option.builder()
            .longOpt(serializeOptionStr)
            .desc("Serialize audio file and send it to the server instead of the decoded samples")
            .build());
    options.addOption(
        Option.builder()
    ...
    if (cmd.hasOption(thresholdOptionStr)) {
      try {
        ...
        System.err.println(
            "Invalid threshold number: '" + cmd.getOptionValue(thresholdOptionStr) + "' ");
        printUsageAndExit(options);
      }
    }
    ...
    if (cmd.hasOption(enrollOptionStr)) {
      ...
      if (!cmd.hasOption(pemOptionStr) && !cmd.hasOption(listOptionStr)) {
    ...
    // Remove a audio enrollment for the specified speaker
      ...
      if (taskList.size() > 0) {
        System.err.println(
            "Enrollment input option is mutually exclusive, all other requests will be ignored");
      }
      ...
      if (!Files.exists(Paths.get(enrollmentImportName).toAbsolutePath())) {
        System.err.println(
            "ERROR: Enrollment model file '" + enrollmentImportName + "' does not exist");
        printUsageAndExit(options);
      }
    }
    ...
    //        if (cmd.hasOption(stereoOptionStr)) {
    //            stereoAudioProcessing = true;
    //            // ignore any channel number - process all channels
    //            channelNumber = -1;
    //            log.info("Frame and Global scoring with stereo files enabled.  Default is to treat
    // audio files as mono.");
    //        }

//    if (cmd.hasOption(videoOptionStr)) {
//      if (dataType == ClientUtils.DataType.IMAGE_DATA){
//        System.err.println(
//                "ERROR: The --image and --video aregument are mutually exclusive.  Pick only one of these options and run again.");
//        printUsageAndExit(options);
//      }
//      dataType = ClientUtils.DataType.VIDEO_DATA;
//    }

    // fixme support sending as file?
    ...
    // Check for a wave, audio vector, or enrollment vector input
    else if (cmd.hasOption(listOptionStr)) {
      ...
      if (cmd.hasOption(wavOptionStr)) {
        System.err.println(String.format("Option '%s' ignored.  Using audio list", wavOptionStr));
      }
    ...
    else if (cmd.hasOption(wavOptionStr)) {
  ...
  // Methods to build Workflow definitions, pretty static/hardcoded to begin

  // Basic SAD and LID workflow
  public Workflow.WorkflowDefinition buildSADandLIDWorkflow(){

    // hack
    Workflow.JobDefinition.Builder workflowJob = Workflow.JobDefinition.newBuilder();
    workflowJob.setDataProperties(Workflow.DataHandlerProperty.newBuilder()
...
//                .setAudio(Olive.Audio.newBuilder().setPath("/tmp/test.wav"))
    ...
    Workflow.WorkflowTask.Builder wfr = Workflow.WorkflowTask.newBuilder()
...
//            .setDataHandler("foobar");

    Workflow.WorkflowDefinition.Builder workflowDef = Workflow.WorkflowDefinition.newBuilder();

    workflowDef.addOrder(order).setActualized(true);

    return workflowDef.build();
  ...
  public void handleEnrollmentRequests(Server server) {
    ...
    try {
      ...
      if(jobTimeout){
        System.err.println("");
        System.err.println("Enrollment failed because job(s) are taking too long to finsh.  Consider increasing timout and trying again.");
        System.exit(2);
      ...
      Server.ResultCallback<Olive.ClassRemovalRequest, Olive.ClassRemovalResult> rc =
          new Server.ResultCallback<Olive.ClassRemovalRequest, Olive.ClassRemovalResult>() {
            ...
            public void call(
              ...
              // do something with the results:
              if (!r.hasError()) {
                log.info("Removed speaker: {}", r.getReq().getClassId());
              } else {
